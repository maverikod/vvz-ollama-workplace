#!/usr/bin/env python3
"""
Read the model list from adapter config (mwps.mwps),
...
def _mwps_list_models() -> set[str]:
    """Return set of model names (first column from 'mwps list')."""
    try:
        out = subprocess.run(
            ["mwps", "list"],
            capture_output=True,
