"""
Unit tests for MwpsProviderClient (step_08).
...
from mwps.mwps_provider_client import (  # noqa: E402
    MwpsProviderClient,
)
...
def _valid_config(
    ...
    """Minimal valid Model Workplace Server provider section."""
    return {
        "transport": {
            "base_url": base_url,
...
def test_validate_config_empty_base_url_raises() -> None:
    ...
    client = MwpsProviderClient(config)
...
def test_validate_config_valid_passes() -> None:
    ...
    client = MwpsProviderClient(_valid_config())
    client.validate_config()
...
def test_supports_tools_true() -> None:
    """MwpsProviderClient.supports_tools() returns True."""
    client = MwpsProviderClient(_valid_config())
