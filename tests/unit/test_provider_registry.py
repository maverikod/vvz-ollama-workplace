...
from mwps.provider_registry import (  # noqa: E402
    get_client,
...
def test_get_client_invalid_config_raises() -> None:
    ...
    invalid = {"default_provider": "mwps", "providers": {}}
    with pytest.raises(ValidationError) as exc_info:
        get_client("mwps", invalid, validate=True)
    assert (
...
def test_get_client_from_app_config_default_provider() -> None:
    ...
    try:
        ...
        # Step_08 may not be done: mwps client not available
        assert "not available" in str(e).lower() or "not implemented" in str(e).lower()


def test_list_supported_providers_includes_mwps() -> None:
    """list_supported_providers returns at least mwps (phase 1)."""
    supported = list_supported_providers()
    assert isinstance(supported, tuple)
    assert "mwps" in supported


def test_get_client_mwps_returns_client_or_raises_clear() -> None:
    """get_client('mwps', valid_config) returns client or clear ValidationError."""
    data = _minimal_mwps()
    try:
        client = get_client("mwps", data, validate=True)
        assert isinstance(client, BaseProviderClient)
    except ValidationError as e:
        assert "not available" in str(e).lower() or "not implemented" in str(e).lower()
