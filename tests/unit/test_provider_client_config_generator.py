...
from mwps.provider_client_config_generator import (  # noqa: E402
    ...
    generate_sample,
    get_default_mwps_provider_section,
)
...
def test_generate_sample_includes_mwps_with_all_subsections() -> None:
    """Sample includes mwps with transport/auth/tls/features/limits per standard."""
    section = generate_sample(validate=True)
    assert section.get("default_provider") == "mwps"
    providers = section.get("providers", {})
    assert "mwps" in providers
    mwps = providers["mwps"]
    assert "transport" in mwps and isinstance(mwps["transport"], dict)
    assert "auth" in mwps and isinstance(mwps["auth"], dict)
    assert "tls" in mwps and isinstance(mwps["tls"], dict)
    assert "features" in mwps and isinstance(mwps["features"], dict)
    assert "limits" in mwps and isinstance(mwps["limits"], dict)
    assert "base_url" in mwps["transport"]
    assert "protocol" in mwps["transport"]


def test_get_default_mwps_provider_section_structure() -> None:
    """Default mwps section has all required and optional keys per standard."""
    section = get_default_mwps_provider_section()
...
def test_generate_provider_clients_section_custom_providers() -> None:
    ...
    assert section["default_provider"] == "mwps"
    assert (
        section["providers"]["mwps"]["transport"]["base_url"]
...
def test_generate_provider_clients_section_default_provider_missing_raises() -> None:
    ...
    with pytest.raises(ValueError, match="must exist in providers"):
        generate_provider_clients_section(
            default_provider="mwps",
            providers={"openai": get_default_mwps_provider_section()},
            validate=False,
        )
