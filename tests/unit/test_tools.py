"""Unit tests for tools.py: three tools, MWPS format, tech spec params."""

import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "src"))  # noqa: E402
from mwps.tools import get_mwps_tools  # noqa: E402


def test_tools_count() -> None:
    tools = get_mwps_tools()
    assert len(tools) == 3
