...
def test_no_alias_returns_none() -> None:
    ...
    assert reg.get_display_name("echo.mwps-adapter", "llama3.2") is None
...
def test_alias_configured_returns_display_name() -> None:
    ...
    reg = CommandAliasRegistry(
        {
            ("echo.mwps-adapter", "llama3.2"): "tool_echo",
        }
    )
    assert reg.get_display_name("echo.mwps-adapter", "llama3.2") == "tool_echo"
...
def test_different_model_no_alias() -> None:
    ...
    assert reg.get_display_name("echo.mwps-adapter", "qwen3") is None
