"""
MWPS tool definitions: list_servers, call_server, help.
...
# MWPS format: type "function", function: name, description, parameters (JSON Schema)
...
def get_mwps_tools() -> List[Dict[str, Any]]:
    """
    Return the three mandatory tools in MWPS-compatible format.

    Each tool has type "function" and function with name, description, parameters.
    Used when building MWPS /api/chat requests.

    Returns:
        List of tool definitions for MWPS tools array.
    """
    return [
        {
            "type": "function",
...
3. Help for a specific command: Call the "help" tool with parameter
   ...
   "mwps_chat") to get the full description and parameters from the server
