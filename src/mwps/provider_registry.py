...
from mwps.provider_client_base import BaseProviderClient
...
def _build_mwps(provider_section: Any) -> BaseProviderClient:
    """
    Build Model Workplace Server provider client from provider section dict.

    Lazy import so step_08 can be merged after step_09. If mwps_provider_client
    ...
    try:
        ...
        raise ValidationError(
            "Provider client for 'mwps' is not available "
            "(mwps_provider_client module not implemented or not installed).",
            cause=e,
        ) from e
    if not isinstance(provider_section, dict):
        raise ValidationError(
            "provider_section for mwps must be a dict, got "
            f"{type(provider_section).__name__}"
        )
    return MwpsProviderClient(provider_section)
...
def get_client(
    ...
    *,
    ...
    Args:
        provider_name: Config key of the provider (e.g. "mwps").
