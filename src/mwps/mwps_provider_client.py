"""
Concrete Model Workplace Server provider client (unified workstation API).

Implements BaseProviderClient for Model Workplace Server: chat, embed, healthcheck, config
...
class MwpsProviderClient(BaseProviderClient):
    """
    Model Workplace Server provider client: chat and embed via Model Workplace Server HTTP API.

    Config is the provider section (e.g. provider_clients.providers.mwps)
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        # Capability flags: Model Workplace Server supports stream, tools, embeddings.
    ...
    def validate_config(self) -> None:
        ...
        if not self._base_url:
            raise ValidationError(
                "Model Workplace Server provider config: transport.base_url is required and non-empty."
            )
        if self._timeout <= 0:
            raise ValidationError(
                "Model Workplace Server provider config: request_timeout_seconds must be positive."
            )

    def healthcheck(self) -> bool:
        """
        Check that the Model Workplace Server endpoint is reachable (GET /api/tags).
        ...
        try:
            ...
            err = httpx.HTTPStatusError(
                "Model Workplace Server healthcheck failed",
                request=resp.request,
                response=resp,
            )
            mapped = self.map_error(err)
            logger.warning(
                "Model Workplace Server healthcheck non-200 status=%s: %s",
                resp.status_code,
                mapped.message,
            )
            return False
            ...
            logger.warning(
                "Model Workplace Server healthcheck failed: %s",
                mapped.message,
                exc_info=False,
            )
            return False
    ...
    def chat(self, request: Any) -> Any:
        """
        Send a chat request to Model Workplace Server (POST /api/chat).
    ...
    def embed(self, request: Any) -> Any:
        """
        Request embeddings from Model Workplace Server (POST /api/embed).
        ...
        if not self._supports_embeddings_flag:
            raise CapabilityNotSupportedError(
                "Model Workplace Server provider client has supports_embeddings=False; "
    ...
    def normalize_response(self, raw_response: Any) -> Any:
        """
        Convert Model Workplace Server /api/chat response to workstation-standard shape.
        ...
        if not isinstance(raw_response, dict):
            raise ProviderProtocolError(
                "Model Workplace Server chat response must be a JSON object.",
                cause=raw_response if isinstance(raw_response, BaseException) else None,
            )
        message = raw_response.get("message")
        if message is None:
            raise ProviderProtocolError(
                "Model Workplace Server chat response missing 'message' field.",
            )
        return {
            "message": message,
    ...
    def _normalize_embed_response(self, raw: Any) -> Any:
        """Convert Model Workplace Server /api/embed response to workstation-standard shape."""
        if not isinstance(raw, dict):
            raise ProviderProtocolError(
                "Model Workplace Server embed response must be a JSON object.",
                cause=raw if isinstance(raw, BaseException) else None,
            )
        embeddings = raw.get("embeddings")
        if embeddings is None:
            raise ProviderProtocolError("Model Workplace Server embed response missing 'embeddings'.")
        return {
    ...
    def map_error(self, exception: BaseException) -> ProviderError:
        ...
        if isinstance(exception, httpx.TimeoutException):
            return ProviderTimeoutError(
                "Model Workplace Server request timed out.",
                cause=exception,
            )
