"""
Thread-safe state for model loading and MWPS readiness.

Model ready: set True only after warm_up_models() in run_adapter (models in memory).
Not set by ping: GET /api/tags only means MWPS is up; model loads on first /api/chat.
...
_message: str = ""
# MWPS responded to ping; server is "ready" only when this is True. Init False.
_model_ready = False
# Runtime default model for new chats; None = use config.mwps_model. Hot-swappable.
...
def is_model_ready() -> bool:
    """Return whether MWPS has responded and model is ready for chat."""
    with _lock:
        return _model_ready
...
def get_active_model() -> str | None:
    """Runtime default model (hot-swappable); None = use config.mwps_model."""
    with _lock:
