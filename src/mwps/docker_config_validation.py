...
VALID_PROVIDER_NAMES: frozenset[str] = frozenset(
    {"mwps", "google", "anthropic", "openai", "xai", "grok", "deepseek", "openrouter"}
)
...
def get_runtime_allowed_providers(app_config: dict) -> set[str]:
    ...
    Authoritative sources: mwps.available_providers, providers
    implied by mwps.mwps.model/models, and
    ...
    if oo:
        om = (oo.get("mwps_model") or "").strip()
        if om:
            providers.add(get_provider_for_model(om))
        for m in oo.get("mwps_models") or []:
...
def get_provider_for_model(model_id: str) -> str:
    """Return provider name for model_id, or 'mwps' if local."""
...
def get_required_api_key_for_model(model_id: str) -> str | None:
    ...
    or None if the model is local/Model Workplace Server (no commercial key required).
    """
    if not model_id or not isinstance(model_id, str):
        return None
    mid = (model_id or "").strip().lower()
...
def _collect_providers_in_use(
    ...
    providers: set[str] = set()
    model_ids: list[str] = []
    if mwps_model and isinstance(mwps_model, str) and mwps_model.strip():
        model_ids.append(mwps_model.strip())
    if isinstance(mwps_models, list):
        for m in mwps_models:
            if isinstance(m, str) and m.strip():
...
def _collect_required_api_keys(
    ow: dict, mwps_model: str | None, mwps_models: list | None
) -> set[str]:
    """
    Collect config key names required for commercial providers that appear
    in the list of available models (mwps_model, mwps_models) or in
...
def validate_model_providers(
    ...
    exact config path (mwps.*).
    Returns list of error messages.
    """
    errors: list[str] = []
    providers = _collect_providers_in_use(
        ow, mwps_model, mwps_models, app_config=app_config
    )
    ...
    oo = _get_mwps_from_ow(ow)
    mwps_url = (oo["mwps_base_url"] or oo["model_server_url"]) if oo else ""

    for prov in providers:
        if prov == "mwps":
            url = ""
            if isinstance(model_providers.get("mwps"), dict):
                url = (model_providers["mwps"].get("url") or "").strip()
            if not url:
                url = mwps_url
            if not url:
                errors.append(
                    "mwps.model_providers.mwps.url or "
                    "mwps.mwps.base_url required when mwps is "
                    "runtime-allowed"
                )
            continue
        ...
        if not url:
            errors.append(
                "mwps.model_providers.%s.url or "
                "mwps.provider_urls.%s required when %s is "
                "runtime-allowed (commercial provider)" % (prov, prov, prov)
            )
        if not api_key:
            key_path = "mwps.%s" % (leg_key or "api_key")
            errors.append(
...
def validate_commercial_model_keys(
    ...
    available models (mwps_model, mwps_models) or in available_providers.
    Returns list of error messages.
    """
    errors: list[str] = []
    required = _collect_required_api_keys(ow, mwps_model, mwps_models)
    for key in required:
        val = ow.get(key)
        if not val or not (isinstance(val, str) and val.strip()):
            errors.append(
                "Provider in available models requires mwps.%s "
...
def validate_project_config(app_config: dict) -> list[str]:
    """
    Project-specific validation (mTLS, mwps.mwps or mwps_server).
    Returns list of error messages; empty if valid.
    When registration.server_id is mwps-server, only mwps base_url is required.
    """
    errors: list[str] = []
    ...
    if server_id == "mwps-server":
        # Minimal: need base_url (mwps_server or mwps.mwps)
        oss = app_config.get("mwps_server") or {}
        ow = app_config.get("mwps") or {}
        oo = ow.get("mwps") if isinstance(ow, dict) else {}
        base = ""
        ...
        if not base:
            errors.append(
                "mwps-server requires mwps_server.base_url or "
                "mwps.mwps.base_url",
            )
        return errors
        ...
        if raw_om is not None and not isinstance(raw_om, list):
            ...
            for i, item in enumerate(om):
                if not isinstance(item, str) or not item.strip():
                    errors.append(
                        "mwps.mwps.models[%s] must be "
                        "non-empty string" % i
                    )
                    break

    # available_providers: optional list of provider names (mwps, google, etc.).
    ...
    if avail_prov is not None:
        if not isinstance(avail_prov, list):
            errors.append("mwps.available_providers must be a list")
        else:
            for i, p in enumerate(avail_prov):
                if not isinstance(p, str) or not p.strip():
                    errors.append(
                        "mwps.available_providers[%s] "
                        "must be non-empty string" % i
                    )
                    break
                if p.strip().lower() not in VALID_PROVIDER_NAMES:
                    errors.append(
                        "mwps.available_providers[%s] must be one of "
        ...
        if val is not None and not isinstance(val, list):
            ...
            for i, item in enumerate(val):
                if not isinstance(item, str):
                    errors.append(
                        "mwps.%s[%s] must be a string" % (key, i)
                    )
                    break
    ...
    if interval is not None:
        try:
            ...
            if val < 0:
                errors.append(
                    "mwps.command_discovery_interval_sec must be >= 0"
                )
        except (TypeError, ValueError):
            errors.append(
                "mwps.command_discovery_interval_sec must be an integer"
            )
    ...
    if rmode is not None and rmode not in ("fixed_order", "unified_by_relevance"):
        errors.append(
            "mwps.relevance_slot_mode must be "
    ...
    if allow_list is not None:
        if not isinstance(allow_list, list):
            errors.append(
                "mwps.model_calling_tool_allow_list must be a list"
            )
        else:
            for i, item in enumerate(allow_list):
                if not isinstance(item, str):
                    errors.append(
                        "mwps.model_calling_tool_allow_list[%s] "
                        "must be a string" % i
                    )
                    break
    ...
    if cname is not None and not isinstance(cname, str):
        errors.append("mwps.mwps.container_name must be a string")
    if cimg is not None and not isinstance(cimg, str):
        errors.append("mwps.mwps.container_image must be a string")
    if (
        cname
        ...
        errors.append(
            "mwps.mwps.container_name requires container_image"
        )
