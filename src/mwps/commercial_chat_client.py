...
Model Workplace Server-style message format for compatibility with chat_flow.
...
def _mwps_to_openai_messages(
    mwps_messages: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    """
    Convert Model Workplace Server message format to OpenAI format.

    Model Workplace Server: role, content, tool_calls (optional), tool_name (for tool role).
    OpenAI: role, content, tool_calls (optional).
    """
    out: List[Dict[str, Any]] = []
    for m in mwps_messages or []:
...
def _openai_to_mwps_message(openai_response: Dict[str, Any]) -> Dict[str, Any]:
    """
    Extract assistant message from OpenAI response to Model Workplace Server format.

    OpenAI: choices[0].message with role, content, tool_calls.
    Model Workplace Server: {role, content, tool_calls}.
    """
...
async def chat_completion(
    ...
    Args:
        ...
        messages: Model Workplace Server-style messages (role, content, tool_calls).
        tools: Optional tool definitions (OpenAI format).
        stream: Whether to stream (not implemented for now).
        timeout: Request timeout in seconds.

    Returns:
        Model Workplace Server-style response: {message: {role, content, tool_calls}, ...}.
    """
    ...
    body: Dict[str, Any] = {
        ...
        "messages": _mwps_to_openai_messages(messages),
        "stream": False,
    }
    if tools:
        body["tools"] = tools
    ...
    msg = _openai_to_mwps_message(data)
    return {
        "message": msg,
