...
def _parse_generate_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        metavar="URL",
        help="MWPS API base URL (default: %(default)s).",
    )
    parser.add_argument(
        "--mwps-model",
        default="llama3.2",
        metavar="NAME",
        help="Default MWPS model (default: %(default)s).",
    )
    parser.add_argument(
        "--mwps-models",
        default="llama3.2",
        metavar="LIST",
        help="Comma-separated mwps_models (default: --mwps-model value).",
    )
    parser.add_argument(
        "--redis-password",
        default=None,
        metavar="PASSWORD",
        help="Optional Redis password for mwps.",
    )
    parser.add_argument(
        "--instance-uuid",
        default=None,
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="ID",
        help="Registration server_id (default: advertised-host or mwps).",
    )
    # Optional mwps overrides
    parser.add_argument(
        "--mwps-timeout",
        type=int,
        default=60,
        metavar="SEC",
        help="MWPS request timeout in seconds (default: %(default)s).",
    )
    parser.add_argument(
        "--max-tool-rounds",
        type=int,
...
def _settings_from_args(args: argparse.Namespace) -> dict:
    ...
    mwps_models_str = getattr(args, "mwps_models", None) or getattr(
        args, "mwps_model", "llama3.2"
    )
    mwps_models = _comma_list(str(mwps_models_str) if mwps_models_str else "")
    ...
    settings: dict = {
        ...
        "mwps_base_url": args.mwps_base_url,
        "mwps_model": args.mwps_model,
        ...
        "mwps_timeout": args.mwps_timeout,
