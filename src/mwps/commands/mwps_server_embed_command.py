"""
Adapter command: embed — Model Workplace Server POST /api/embed (mwps-server surface).
...
class MwpsServerEmbedCommand(Command):
    """
    Call Model Workplace Server POST /api/embed with model and input (string or list of strings).
    Returns embeddings array and optional model, prompt_eval_count.
    """

    name = "embed"
    descr = (
        "Model Workplace Server embed: POST /api/embed with model and input "
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        """POST to Model Workplace Server /api/embed; return embeddings."""
        ...
        try:
            ...
            logger.warning("mwps-server embed HTTP error: %s", msg)
            return ErrorResult(message="Model Workplace Server embed failed: %s" % msg, code=-32603)
        except Exception as e:
            logger.warning("mwps-server embed failed: %s", e)
            return ErrorResult(message="Model Workplace Server embed failed: %s" % e, code=-32603)
