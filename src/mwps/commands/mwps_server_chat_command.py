"""
Adapter command: chat — Model Workplace Server POST /api/chat (mwps-server surface).

Single request/response; no tool loop. Used when adapter registers as mwps-server.
...
from ..mwps_server_config import get_mwps_server_settings
...
class MwpsServerChatCommand(Command):
    """
    Call Model Workplace Server POST /api/chat with model and messages. No tool loop.
    Returns message, prompt_eval_count, eval_count.
    """

    name = "chat"
    descr = (
        "Model Workplace Server chat: POST /api/chat with model and messages. "
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        """POST to Model Workplace Server /api/chat; return normalized response."""
        config_data = _get_config_data()
        base_url, timeout = get_mwps_server_settings(config_data)
        ...
        try:
            ...
            logger.warning("mwps-server chat HTTP error: %s", msg)
            return ErrorResult(message="Model Workplace Server chat failed: %s" % msg, code=-32603)
        except Exception as e:
            logger.warning("mwps-server chat failed: %s", e)
            return ErrorResult(message="Model Workplace Server chat failed: %s" % e, code=-32603)
