"""
Adapter command: direct_chat — call MWPS /api/chat only, no session/tools/context.
...
class DirectChatCommand(Command):
    """
    Call MWPS /api/chat with one user message. No session, Redis, tools, context.
    Returns assistant reply and duration_sec to see if the bottleneck is the model.
    """

    name = "direct_chat"
    descr = (
        "Direct call to MWPS /api/chat with a single user message. "
    ...
    async def execute(self, **kwargs: Any) -> Any:
        """POST to MWPS /api/chat only; return reply and duration."""
        ...
        endpoint = resolve_model_endpoint_from_provider_clients(
            ...
            model,
            default_model=config.mwps_model or "llama3.2",
        )
        timeout = max(30.0, float(config.mwps_timeout or 60))
