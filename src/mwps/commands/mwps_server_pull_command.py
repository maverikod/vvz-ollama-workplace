"""
Adapter command: pull — Model Workplace Server POST /api/pull (mwps-server surface).
...
class MwpsServerPullCommand(Command):
    """
    Call Model Workplace Server POST /api/pull to download a model by name.
    ...
    name = "pull"
    descr = (
        "Model Workplace Server pull: POST /api/pull to download a model. "
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        """POST to Model Workplace Server /api/pull; return status."""
        ...
        try:
            ...
            logger.warning("mwps-server pull HTTP error: %s", msg)
            return ErrorResult(message="Model Workplace Server pull failed: %s" % msg, code=-32603)
        except Exception as e:
            logger.warning("mwps-server pull failed: %s", e)
            return ErrorResult(message="Model Workplace Server pull failed: %s" % e, code=-32603)
