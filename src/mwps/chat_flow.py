"""
Core chat loop: MWPS /api/chat with tools, proxy execution, tool messages.
...
def _tool_message(tool_name: str, content: str) -> Dict[str, Any]:
    """Build MWPS tool message: role tool, tool_name, content (spec 2.2)."""
    return {
        "role": "tool",
        "tool_name": tool_name,
        "content": content,
...
async def run_chat_flow(
    ...
    if representation is None:
        from .mwps_representation import MwpsRepresentation

        representation = MwpsRepresentation()
    use_max_rounds = (
    ...
    for round_num in range(use_max_rounds):
        ...
        try:
            ...
            logger.info(
                "chat_flow MWPS round=%s duration_sec=%.3f",
                round_num + 1,
                time.perf_counter() - t_mwps_start,
            )
            ...
            logger.exception(
                "chat_flow MWPS request failed round=%s error=%s",
                round_num + 1,
                e,
            )
            if total_prompt_tokens or total_eval_tokens:
