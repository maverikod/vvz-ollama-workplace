...
def resolve_model_endpoint(
    ...
    Args:
        ...
        config: Workstation config (for keys and mwps URL).
        provider_urls: Optional override for provider -> base_url.
        openrouter_api_key: Optional OpenRouter key (env or config).

    Returns:
        ModelEndpoint with base_url, api_key, provider, model_id, is_mwps.
    """
    if not model_id or not isinstance(model_id, str):
        model_id = (config.mwps_model or "").strip() or "llama3.2"
    model_id = model_id.strip()
    provider = get_provider_for_model(model_id)

    # Model Workplace Server: url and optional api_key from model_providers or legacy.
    if provider == "mwps":
        url, key = _get_provider_from_config(config, "mwps")
        base = url or (
            (config.model_server_url or config.mwps_base_url) or ""
        ).rstrip("/")
        if not base:
            base = DEFAULT_PROVIDER_URLS.get("mwps", "http://127.0.0.1:11434")
        return ModelEndpoint(
            base_url=base,
            api_key=key or config.mwps_api_key,
            provider="mwps",
            model_id=model_id,
            is_mwps=True,
        )
