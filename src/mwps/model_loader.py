...
Uses model server HTTP API (GET /api/tags, POST /api/pull). No subprocess mwps CLI.
...
def _mwps_list_models_via_api(base_url: str, timeout_sec: float = 30.0) -> Set[str]:
    ...
    try:
        ...
        logger.warning("_mwps_list_models_via_api failed: %s", e)
    return names
...
def _mwps_pull_via_api(
    ...
    try:
        ...
        logger.warning("_mwps_pull_via_api %s failed: %s", model, e)
        return False
...
def run_model_loading_via_api(
    ...
    present = _mwps_list_models_via_api(base_url)
    pulled = 0
    ...
    for model in model_names:
        ...
        if _mwps_pull_via_api(base_url, model, timeout_sec=pull_timeout_sec):
            present.add(model)
            pulled += 1
            logger.info(
...
def run_model_loading(config_path: str) -> None:
    ...
    Call from a background thread; uses API only (no mwps CLI).
    """
    set_ready()
...
def warm_up_models(
    ...
    """
    Send a minimal chat request per model so MWPS loads it into memory.
    Run before starting the adapter server so the first mwps_chat is not cold.
    """
    if not model_names or not (mwps_base_url or "").strip():
        return
    base = (mwps_base_url or "").rstrip("/")
    try:
        import httpx
    except ImportError:
