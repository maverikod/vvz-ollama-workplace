...
config with transport (ws, fallback), security, limits, log paths, runtime
identity. Supports prompt-driven generation via merge of template, env, and args.
...
def _default_template(certs_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "registration_server_id": "model-workspace-server",
        "registration_server_name": "Model Workspace Server",
...
def _env_overlay() -> dict[str, Any]:
    ...
    if certs_dir:
        ...
        out["server_cert_file"] = str(p / "server" / "test-server.crt")
        out["server_key_file"] = str(p / "server" / "test-server.key")
        out["server_ca_cert_file"] = str(p / "ca" / "ca.crt")
        out["registration_cert_file"] = str(p / "client" / "test-server.crt")
        out["registration_key_file"] = str(p / "client" / "test-server.key")
        out["registration_ca_cert_file"] = str(p / "ca" / "ca.crt")
    out["output_path"] = os.environ.get("MODEL_WORKSPACE_SERVER_CONFIG_PATH") or ""
    port = os.environ.get("MODEL_WORKSPACE_SERVER_PORT")
    if port:
        try:
...
def generate_server_config(settings: dict[str, Any]) -> None:
    """
    Generate adapter config for model workspace server.

    Uses adapter SimpleConfigGenerator for base, then applies server/transport
    and model_workspace_server section (transport ws, fallback, limits, logs,
    ...
    client_cert = str(
        ...
        or certs_dir / "client" / "test-server.crt"
    )
    client_key = str(
        settings.get("registration_key_file")
        or certs_dir / "client" / "test-server.key"
    )
    reg_ca = str(
    ...
    generator.generate(
        ...
        registration_server_name=str(
            settings.get("registration_server_name", "Model Workspace Server")
        ),
    ...
    data["transport"] = {
        "verify_client": True,
        "transport_type": str(settings.get("transport_type", "ws")),
        "fallback_policy": str(settings.get("fallback_policy", "deny")),
    }
    data["model_workspace_server"] = {
        "runtime_identity": {
            "instance_uuid": instance_uuid,
            "server_id": server_id,
        },
...
def generate_from_merged(
    ...
    if not settings.get("output_path"):
        settings["output_path"] = Path("config") / "model_workspace_server_config.json"
