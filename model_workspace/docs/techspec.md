...
This document describes **only the model workspace** application: its role, tools, chat flow, and configuration. It does not describe Redis, OLLAMA, or the MCP proxy in detail; those are separate components (redis_adapter, ollama_adapter, proxy). See [ТЗ.md](ТЗ.md) and root [SPEC](../../docs/plans/refactoring_adapter_structure/SPEC.md) for the five subprojects (incl. provider clients); Ollama = one provider.
...
- Builds **context** (system, history, relevance slot, current message) and sends a **chat request** to the model via a **provider client** (e.g. **ollama_provider_client**). **For the model workspace, Ollama server is just a separate provider** (SPEC §5).
...
So: the workspace **orchestrates** the model (via provider clients such as ollama_provider_client) and the proxy (via proxy client). It has **no** Redis or Ollama inside; it uses **ollama_provider_client** and **redis_provider_client** (SPEC §4, §5, §8).
...
- **In scope:** Design and implementation of the workspace: (1) context building, (2) calling **provider clients** (e.g. ollama_provider_client for chat/embed), (3) injecting proxy tools into requests, (4) handling tool_calls via the proxy client, (5) configuration (proxy URL, provider_clients config, model name, limits). Documentation and a minimal example.
- **Out of scope:** Implementation of redis_adapter, ollama_adapter, ollama_provider_client, redis_provider_client, or MCP proxy; modifying Redis or Ollama. Other backends are supported via their provider client packages and config.

### 1.3 Goals

1. The model (reached via **provider client**, e.g. ollama_provider_client) can **list** servers and **call** commands on any server registered in the MCP Proxy (via proxy client).
2. Single place of configuration for the **workspace**: MCP Proxy URL, provider_clients config (Ollama = one provider), default model, timeouts, context limits.
...
2. The workspace builds **context** (see [context_formation.md](context_formation.md)), then builds a chat request for the **provider client** (e.g. ollama_provider_client) with: `model`, `messages`, optional `stream`, and `tools` = list_servers, call_server, help in the provider’s format.
...
- **provider_clients** — config for each provider (Ollama = one provider; connection to ollama-adapter or Ollama, model name, timeouts); see root [provider_client_config_standard](../../docs/standards/provider_client_config_standard.md).
- **ollama_model** (or default model key) — default model name for chat.
- **ollama_timeout** — timeout in seconds for chat requests (optional).
...
- **Reuse:** Use the same proxy client for MCP Proxy calls; use **provider clients** (ollama_provider_client, redis_provider_client) for chat/embed and storage; do not duplicate adapter or proxy protocol in the workspace.
...
- **Model workspace** — the application that orchestrates the model (via provider clients) and the MCP Proxy (via proxy client), builds context, and handles tool_calls (SPEC §4). **Ollama is just one provider** for the workspace.
- **Provider client** — package that implements the uniform API (chat, embed, healthcheck, errors) for one provider; e.g. **ollama_provider_client** (Ollama), **redis_provider_client** (storage). See root [provider_client_standard](../../docs/standards/provider_client_standard.md) and [SPEC §5, §8](../../docs/plans/refactoring_adapter_structure/SPEC.md).
