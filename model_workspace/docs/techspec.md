...
- Builds **context** (system, history, relevance slot, current message) and sends a **chat request** to the model via the **client to ollama-adapter** (SPEC §6).
...
So: the workspace **orchestrates** the model (via client to ollama-adapter) and the proxy (via proxy client). It has **no** Redis or Ollama inside; it uses **client to redis-adapter** and **client to ollama-adapter** (SPEC §4, §8).
...
- **In scope:** Design and implementation of the workspace: (1) context building, (2) calling the **client to ollama-adapter** for chat (and optionally embed), (3) injecting proxy tools into requests, (4) handling tool_calls via the proxy client, (5) configuration (proxy URL, adapter client config, model name, limits). Documentation and a minimal example.
- **Out of scope:** Implementation of redis_adapter, ollama_adapter, or MCP proxy; modifying Redis or Ollama. Other backends are supported only via their adapter clients and config.

### 1.3 Goals

1. The model (reached via **client to ollama-adapter**) can **list** servers and **call** commands on any server registered in the MCP Proxy (via proxy client).
2. Single place of configuration for the **workspace**: MCP Proxy URL, client-to-ollama-adapter config (URL via proxy), default model, timeouts, context limits.
...
2. The workspace builds **context** (see [context_formation.md](context_formation.md)), then builds a chat request for the **provider client** (e.g. ollama_adapter) with: `model`, `messages`, optional `stream`, and `tools` = list_servers, call_server, help in the provider’s format.
...
- **Client to ollama-adapter config** — connection to ollama-adapter via proxy (server id, model name, timeouts); see root [provider_client_config_standard](../../docs/standards/provider_client_config_standard.md).
...
- **Reuse:** Use the same proxy client for MCP Proxy calls; use the **client to ollama-adapter** for chat/embed; do not duplicate adapter or proxy protocol in the workspace.
...
- **Model workspace** — the application that orchestrates the model (via client to ollama-adapter) and the MCP Proxy (via proxy client), builds context, and handles tool_calls (SPEC §4).
- **Client to ollama-adapter** — the ollama_adapter client used by the workspace to call Ollama API (chat, embed, etc.) over WebSocket via proxy; see root [provider_client_standard](../../docs/standards/provider_client_standard.md) and [SPEC §6](../../docs/plans/refactoring_adapter_structure/SPEC.md).
