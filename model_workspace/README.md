...
Model workspace subproject: workstation application using **provider clients** (ollama_provider_client, redis_provider_client). For the model workspace, **Ollama server is just a separate provider**. No Redis or Ollama inside. PyPI-publishable layout.
