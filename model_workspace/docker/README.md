...
Scripts to **build the image** and **create/run the container** for the model-workspace-server (SPEC §4). This subproject has no Redis or Ollama inside; it uses clients to redis-adapter and ollama-adapter.
...
| Script | Purpose |
|--------|---------|
| **build_image.sh** | Build Docker image from repository root (needs ollama_adapter and redis_adapter in repo). |
