# Stack: MWPS + Redis + Adapter containers

**Author:** Vasiliy Zdanovskiy  
**Email:** vasilyvz@gmail.com  

This document describes running the **full stack** per [SPEC §4](plans/refactoring_adapter_structure/SPEC.md): containers **redis-adapter** (Redis + adapter server), **mwps-adapter** (Model Workplace Server + adapter server), and **model-workspace-server** (model workspace app only). The model workspace has **no** Redis or Model Workplace Server inside; it uses **clients** to redis-adapter and mwps-adapter (WebSocket via proxy).

**Legacy / single image:** Some setups use one image that includes Redis, MWPS, and the project server (MCP adapter). Bases, configs, logs, and models are mounted from the host. The run script uses user **1000:1000**, **restart=always**, and connects the container to the **smart-assistant** network.

## Use MWPS only from the stack

When running the MWPS + adapter stack in Docker, **stop the local MWPS** so that only the stack’s MWPS is used (the adapter inside the container talks to `http://127.0.0.1:11434` or to the mwps_adapter service).

Stop local MWPS:

```bash
# If running as a service
sudo systemctl stop mwps

# If started manually
pkill -f "mwps serve"
```

Then run the container(s); MWPS will start inside the stack and the adapter will use it.

If **mwps_chat** returns 404, the requested model is usually not pulled. Either pull it once (e.g. `docker exec <container> mwps pull llama3.2`) or set `MWPS_PULL_MODEL=1` when running the container so the default model is pulled on first start (slow on first run).

**Empty message or timeout:** If the model reply is empty or the request stops after ~60s, increase `mwps_timeout` in the workstation config (e.g. 120). Cold model or long context can exceed 60s. In container logs, a `500` on `POST /api/chat` after ~1m usually means MWPS hit an internal timeout or error; retry or increase timeouts.

**SERVER_UNAVAILABLE from proxy:** Often means the adapter raised an error before returning. Check adapter logs (`docker logs <container>`). If you see `Command timed out after 120.00s` (or similar), the command (e.g. `mwps_chat`) exceeded the adapter’s execution timeout. Increase the command timeout in the adapter config (e.g. 300) and restart the container; also ensure `mwps_timeout` is high enough for slow or cold model responses.

**No cold start:** The adapter may run model loading and **warm-up** (pull missing models, then one minimal chat per model) **before** starting the HTTP server. So the first `mwps_chat` is served when the model is already loaded.

**Keep model in memory:** The entrypoint may set `MWPS_KEEP_ALIVE=-1` by default (never unload). Override with env when running (e.g. `-e MWPS_KEEP_ALIVE=30m` for 30 minutes idle).
...
| **mwps_adapter**  | `./docker/build_image.sh` (from mwps_adapter/)  | `./docker/run_container.sh` | mwps-adapter            |
...
**Full stack order:** Create network `smart-assistant`, then build and run: 1) redis-adapter, 2) mwps-adapter, 3) model-workspace-server. Each script mounts config, logs, certs (from repo `mtls_certificates/` when present). See each subproject’s **docker/README.md**.

**Legacy (single image):** From the project root, `./docker/build_and_run.sh` still builds one image (Redis + MWPS + adapter) for backward compatibility. Prefer the per-subproject scripts above for the target three-container layout (SPEC §4).

To use an existing MCP proxy on another container, put both on the same network and set the proxy host (e.g. service name):

```bash
docker network create mwps-net
docker run -d --name mcp-proxy --network mwps-net -p 3004:3004 your-mcp-proxy-image
docker run --rm --network mwps-net -p 8443:8443 -e MCP_PROXY_HOST=mcp-proxy mwps-adapter
```
...
1. **Generate certs** (CN = container name from `docker/run.conf`, e.g. `mwps-adapter`):
   ```bash
...
A **test** container with name suffix `-test` (e.g. `mwps-adapter-test`) uses the **same image** but mounts project code from the host so you can change code without rebuilding the image.
...
- Container name: `mwps-adapter-test` (set in `docker/run.conf` as `CONTAINER_NAME_TEST`).
...
- No `restart=always`; after code changes, restart with: `docker restart mwps-adapter-test`.
...
In config set `available_providers` (e.g. `["mwps", "google"]`) and the required key; use the model id in sessions or as default model.
