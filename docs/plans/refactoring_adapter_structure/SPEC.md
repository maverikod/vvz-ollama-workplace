...
**Принцип клиентов:** Все клиенты должны быть **на базе клиента адаптера** (mcp-proxy-adapter: WebSocket, протокол команд). В клиенте **скрыта специфика и формат** работы с конкретным провайдером/сервисом: вызывающий код видит только единый API (Ollama-методы, Redis-команды, контракт provider_client_standard и т.д.), транспорт и сериализация — внутри клиента.
...
|---|-----------|------------|------------|
| 1 | Ollama сервер + адаптер | **ollama_adapter** | Сервер: Ollama + API на базе mcp-proxy-adapter; регистрация на прокси; доступ к Ollama только через адаптер. |
...
| 3 | Рабочее место (сервер) | **model_workspace** | Приложение «рабочее место модели»: оркестрация чата, сессии, контекст, инструменты через MCP Proxy; использует клиенты провайдеров; внутри нет Redis и Ollama. |
| 4 | Клиент Ollama на базе клиента адаптера | **ollama_provider_client** | Клиент провайдера Ollama: единый API (chat, embed, healthcheck) для model_workspace; **на базе клиента адаптера**; внутри скрыты формат и транспорт к ollama-adapter. Для рабочего места Ollama — просто отдельный провайдер. |
...
- **ollama_provider_client:** клиент провайдера Ollama **на базе клиента адаптера**; скрывает формат и транспорт; для рабочего места **Ollama — просто отдельный провайдер**; контракт по docs/standards/provider_client_standard.
...
- **model-workspace-server:** только логика рабочего места; использует клиенты провайдеров (ollama, redis, при необходимости другие); тоннельный режим через эти клиенты.
