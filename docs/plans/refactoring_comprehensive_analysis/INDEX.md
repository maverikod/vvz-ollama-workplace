...
**Scope:** `src/mwps/`, `src/model_workspace_client/`, `src/model_workspace_server/`, `scripts/`
...
| [STEP-03](step-03_mwps_chat_command.md) | `commands/mwps_chat_command.py` | 476 lines, dup schemas | STEP-01 |
...
| [STEP-09](step-09_mwps_provider_client.md) | `mwps_provider_client.py` | dup normalize methods | STEP-08 |
...
|------|---------------|------|------------|
| [STEP-P1](step-p1_representation_registry_enforcement.md) | `chat_flow.py`, `mwps_chat_command.py`, `get_model_context_command.py`, `tools.py` | Fix P1+P5: route all representation through `RepresentationRegistry`; rename `get_mwps_tools` → `get_tools_for_model(model_id)` | — |
| [STEP-P2](step-p2_provider_registry_and_base.md) | `provider_registry.py`, `provider_client_base.py` | Fix P2+P3: implement `get_client()` for all 6 providers; remove `MwpsServer*Command` direct HTTP; enforce `BaseProviderClient` contract | STEP-08 |
...
|------|------|------|------------|
| [STEP-C1](step-c1_config_migration.md) | `config.py`, `WorkstationConfig` | Remove `mwps_url`/`mwps_model`; parse `provider_clients` structure per standard; add per-provider validation V-PROV-1..6 | STEP-08 |
