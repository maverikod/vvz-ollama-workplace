# STEP-09 — mwps_provider_client.py: eliminate normalize_response duplicate

**File:** `src/mwps/mwps_provider_client.py`  
...
|--------|-------|----------------|
| `normalize_response` | 206–227 (22) | Convert chat response to Model Workplace Server format |
| `_normalize_embed_response` | 229–243 (15) | Convert embed response to Model Workplace Server format |

## Problem

Both methods perform the same structural operation: take a raw provider response dict,
extract fields, build a normalized Model Workplace Server-format dict. Detected similarity is 1.0 —
