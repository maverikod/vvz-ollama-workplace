# STEP-03 — mwps_chat_command.py: split + deduplicate schema methods

**File:** `src/mwps/commands/mwps_chat_command.py`  
...
```
These should each live in a dedicated `mwps_chat_schema.py` (already exists — verify if it's used).
...
### 3a. Move schemas to mwps_chat_schema.py

`mwps_chat_schema.py` already exists. Move the three schema dicts there as module-level constants or functions. Import them in `MwpsChatCommand`.
...
- [ ] Schema dicts moved to `mwps_chat_schema.py`, imported back
