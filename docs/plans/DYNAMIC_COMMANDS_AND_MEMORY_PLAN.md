...
| | `SessionInitCommand` | Adapter command: creates session, returns session_id; accepts optional model. |
...
| **Memory / index** | `MemoryIndexWorker` | Reads unprocessed messages from Redis; calls **chunker** and **vectorizer** (separate services) via **existing clients**; writes vector table, updates vector index and BM25 store; marks processed. See §4.2a. |
...
- Update strategy: to be decided (startup, periodic, lazy, on-demand, or combination).
...
- Messages are written to Redis (see §3) with **session_id**. A **separate process** (worker) consumes unvectorized data and calls **external chunker and vectorizer services** (separate services; **existing clients** are used). The chunker returns text chunks and BM25 tokens; the vectorizer returns embeddings per chunk.
...
- **Chunker and vectorizer:** **Separate external services**; **ready-made clients** exist. The worker uses these clients (no new API to define). **Chunker service** — input: message text, metadata; output: text chunks, BM25 tokens (per chunk or per message). **Vectorizer service** — input: text (e.g. chunks); output: embeddings (vectors) per chunk.
- **Worker flow:** (1) Read unprocessed messages from Redis; (2) for each message, call **chunker** → chunks and BM25 tokens; call **vectorizer** → vectors per chunk; (3) write **vector table**: (chunk_id, vector) — chunk and vector are in DB first; (4) **add to FAISS** → FAISS returns integer **vector_index_id** → update row with vector_index_id; (5) update **BM25/chunk store**: chunk_id + tokens (and message_id for linkage); (6) mark message as processed in Redis.
...
- **Redis and index:** Redis holds raw message stream; worker consumes, calls **chunker** and **vectorizer** (separate services, **existing clients**), updates **vector index** (e.g. FAISS) and **BM25/chunk store**; index update incremental or batch (§4.2a).
- **Search methods:** **Semantic search** (by query embedding, session_id, top_k) and **BM25 search** (by query tokens, session_id, top_k); both return chunks with scores; used to fill relevance slot; implementation may use one or both, optionally hybrid (§4.2b).
- Semantic memory: Redis → worker → chunker service (chunks, BM25) + vectorizer service (embeddings); worker uses existing clients; UUID per message; link to session.
...
| Area | Covered | Notes |
|------|---------|--------|
| **Dynamic commands** | Yes | Discovery from proxy, Command.ServerId, allow/forbidden/policy; safe names and registry; aliases per model; update strategy left to choose. Session store (DB) with model, allowed/forbidden per session; add/remove command to session (reject + log if config-forbidden); config overrides session. |
...
| **Semantic memory** | Yes | Worker + chunker and vectorizer (separate services, existing clients) → BM25, vectors, chunks; message UUID; link to session. |
...
- **Update strategy for commands** (§1.2) — not decided: startup, periodic, lazy, on-demand, or combination. Needs one chosen option (or hybrid) before implementation.
- **Exact Redis layout** — key format, STREAM vs LIST, TTL, retention. Planned for implementation phase.
- **Standards / session rules** — where they are stored (config, Redis, API), how they are loaded, and how their token size is computed for remainder. Mentioned but not specified.
...
- **Chunker and vectorizer** — separate external services; **existing clients** are used; worker does not define a new API. Auth, retries, and behaviour when a service is unavailable to be aligned with those clients.
- **Session-init command** — name, parameters (e.g. **model** to set the session’s model for context size), response shape (session_id in body); whether it is a new adapter command or part of an existing one. Not specified.
- **Session-update command** — **SessionUpdateCommand** added in step_13 to set model and update session lists; session-init name and response shape TBD (see [dynamic_commands_memory/14_ambiguities_and_open_points.md](dynamic_commands_memory/14_ambiguities_and_open_points.md)).
...
- **Dependency on chunker and vectorizer** — separate external services with existing clients; if either is down or slow, semantic slot may be empty or delayed; retry/backoff and behaviour (e.g. context without semantics, or fail request) follow client capabilities.
...
3. **Semantic memory** — Messages are stored in Redis; a worker uses **chunker** and **vectorizer** (separate services, existing clients) to produce chunks, BM25 tokens, and embeddings per message (by UUID). Retrieval is by semantics, then filtered by current session, so the model gets relevant past content within the same dialogue.
...
- Worker process that reads from Redis, calls **chunker** and **vectorizer** (separate services, existing clients), and stores/indexes vectors and chunks per message (and session).
...
**Conclusion:** The plan is coherent and covers dynamic commands, Redis stream, session, semantic memory, context window (with **unified relevance slot** for documentation vs. conversation), documentation access (TOC + get content, pluggable source), and tool→model recursion. **Weak point addressed:** priority between documentation and conversation is not fixed; we compare vector distance of (documentation, current message) and (session, current message) and take the **smaller distance** (more relevant), except for the N messages which are always included. Same embedding space required for fair comparison. Main open work: (1) choose command list update strategy, (2) define Redis key/stream layout (chunker and vectorizer use existing clients), (3) define storage and token-sizing for standards/session rules and parameter N, (4) define session-init command contract, (5) implement unified relevance merge (one embedding model for both session and doc chunks). Implementation can proceed in phases (e.g. Redis + writer → session + context build → worker + chunker and vectorizer (existing clients) → dynamic commands → call stack + depth → unified relevance slot).
